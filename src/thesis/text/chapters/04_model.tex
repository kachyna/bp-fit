...
$$\text{S}_\text{inference} = \text{P}_\text{token} \cdot \text{Max Tokens} \cdot \text{k}_\text{utilization},$$

kde $\text{P}_\text{token}$ je průměrná cena za zpracování jednoho tokenu, $\text{Max Tokens}$ je maximální počet zpracovatelných tokenů a $\text{k}_\text{utilization}$ je koeficient využití. Následně se HPH pro inferenční centrum $\text{HPH}_\text{inference}$ vypočítá jako rozdíl mezi tržbami a mezispotřebou:
