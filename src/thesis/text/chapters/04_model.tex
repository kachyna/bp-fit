...
Analýza českých datových center dále ukázala poměry mezi velikostí budovy a pozemku (0,7274, n=10), velikostí whitespace a budovy (0,4698, n=6) a instalovaným příkonem a velikostí whitespace (0,0030 $\text{MW} / m^2$, n=17). Dále se ukázalo, že průměrná velikost pozemku pro datové centrum o instal. příkonu 1~MW je 2998,0 $m^2$ (n=26). Tyto hodnoty byly vypočítány pouze z neodhadovaných dat, aby nedošlo k jejich zkreslení. V modelu budou následně použity pro odhad velikosti DC pro výpočet daně z nemovitosti.
...
\begin{description}
	\item[Instalovaný příkon] Instalovaný příkon (IT Power) určuje škálu uvažovaného datového centra. Do modelu vstupuje jako určující veličina pro velikost provozu a následně ovlivňuje zejména energetické výstupy (spotřebu elektřiny) a od nich odvozené environmentální dopady.
...
Kvůli tomu byly do modelu implementovány tři scénáře: pesimistický, optimistický a realistický. Tyto scénáře jsou vztaženy k ekonomické části, tedy k příjmům do veřejného rozpočtu a vytvořené hrubé přidané hodnotě. To se však může vůči ostatním oblastem projevit inverzně -- nižší spotřeba energie sice způsobí nižší přidanou hodnotu (protože přidaná hodnota DC je proměna energie na informace), ale na druhou stranu tím vyvolá méně emisí oxidu uhličitého do atmosféry a spotřebuje se méně vody. Ekonomická oblast byla zvolena jako kotvicí kvůli tomu, že se obecně jedná o primární determinant rozhodovacího procesu jak soukromých investorů, tak státního aparátu.
...
kde $\text{I}_\text{building per MW}$ jsou průměrné investiční náklady na výstavbu datového centra s 1~MW instalovaného příkonu.
...
Při srovnávání s jinými sektory a obecně dalšími metrikami je však problém, že náklady na IT vybavení se odepisují rychleji než samotná budova, kvůli čemuž se v průběhu času mění jejich váha v celkových nákladech. Z tohoto důvodu byl zaveden další parametr odpisů, které stanovují délku odpisování IT vybavení $\text{UL}_\text{equipment}$ a délku odpisování budovy $\text{UL}_\text{building}$. Celkové roční odpisy $\text{D}_\text{y}$ se pak vypočítají jako

$$\text{D}_\text{y} = \frac{\text{I}_\text{equipment}}{\text{UL}_\text{equipment}} + \frac{\text{I}_\text{building}}{\text{UL}_\text{building}},$$

kde $\text{D}_\text{y}$ jsou celkové roční odpisy, $\text{I}_\text{equipment}$ jsou náklady na IT vybavení, $\text{UL}_\text{equipment}$ je délka odpisování IT vybavení, $\text{I}_\text{building}$ jsou náklady na výstavbu budovy a $\text{UL}_\text{building}$ je délka odpisování budovy. Alternativní přístup by spočíval ve vypočtení celkových nákladů za životnost projektu a jejich následném rozdělení do jednotlivých let, čímž by se v tomto zjednodušeném modelu dospělo ke stejnému výsledku. Tato metrika se nazývá anualizovaná investice (anglicky annualized CAPEX).
...
Daň z nemovitosti se odvíjí od velikosti pozemku a budovy, na kterých se datové centrum nachází, a od sazeb daně z nemovitosti, které jsou stanoveny pro danou lokalitu. Pro výpočet daně z nemovitosti $\text{T}_\text{property}$ se použijí následující vzorce:
...
$$\text{HPH}_\text{construction} = \text{I}_\text{total} \cdot \text{k}_\text{GVA on Construction Investment},$$

kde $\text{HPH}_\text{construction}$ je hrubá přidaná hodnota z výstavby, $\text{I}_\text{total}$ je celkový investovaný kapitál pro výstavbu a $\text{k}_\text{GVA on Construction Investment}$ je koeficient pro výpočet HPH z investovaných nákladů.
