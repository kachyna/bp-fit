...
$$\text{S}_\text{training} = \text{Price}_\text{training} \cdot \text{GPU Count} \cdot \text{k}_\text{utilization} \cdot \text{k}_\text{occupancy},$$
...
$$\text{S}_\text{inference} = \text{Price}_\text{token} \cdot \text{Max Tokens} \cdot \text{k}_\text{utilization} \cdot \text{k}_\text{occupancy},$$

kde $\text{Price}_\text{token}$ je průměrná cena za zpracování jednoho tokenu, $\text{Max Tokens}$ je maximální počet zpracovatelných tokenů a $\text{k}_\text{utilization}$ je koeficient využití a $\text{k}_\text{occupancy}$ je koeficient obsazenosti. Následně se HPH pro inferenční centrum $\text{GVA}_\text{inference}$ vypočítá jako rozdíl mezi tržbami a mezispotřebou:
