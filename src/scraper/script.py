...
OUTPUT_FILE = "datacentermap_czech_republic.csv"
HEADLESS = False
SLOW_MODE = False
...
def get_page(page, url):
    ...
    while "You're in the Right Place" in soup.get_text():
...
def get_dc_data(page, link, market):
    try:
        ...
        # Name fallback: try h2.customheader, then h1
        ...
        data = {
            ...
            "address": op_link.parent.text.strip().replace(op_link.text.strip(), "").strip(", ") if op_link else "",
            "market": market['name'],
            "url": link
        }
...
def scrape():
    with sync_playwright() as p:
        ...
        # Final CSV write
        if results:
            core_fields = ["facility_name", "operator", "address", "market", "url"]
